...
class Basic_Strategy:
    ...
    def run_match(self,player,dealer,player_sum,dealer_sum,split):
        ...
        if not (6 in player and 2 in player):
            round_hard_doubling[8]=[5,6]
        if split==False:
            round_soft_doubling[1] =[5,6]
        ...
        if  player_sum==16 and not (10 in player and 6 in player) and not (9 in player and 7 in player):
            round_hard_standing[16] = [2,3,4,5,6,10]
        if  player[0]==7 and player[1]==7:
            round_hard_standing[14] = [2,3,4,5,6,10]
