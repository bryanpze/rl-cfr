...
class KuhnPoker:
    ...
    def cfr(self,history,p0,p1,iter):
        ...
        # If iter is even, p1 is the traverser, p2 is fixed and vice versa
        ...
        if even:
            if player==0:
                ...
                next_action = 'p' if node_strategy[0]>random.random() else 'b'
                next_history = history+next_action
                node_util+=-self.cfr(next_history,p0,p1,iter)
            return node_util
        else:
            if player==1:
                util = np.zeros(self.num_actions)
